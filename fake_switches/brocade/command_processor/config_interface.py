...
class ConfigInterfaceCommandProcessor(BaseCommandProcessor):
    ...
    def get_prompt(self):
        return u"SSH@%s(config-if-e1000-%s)#" % (self.switch_configuration.name, split_port_name(self.port.name)[1])
    ...
    def do_vrf(self, *args):
        if u"forwarding".startswith(args[0]):
            if len(args) > 1:
                ...
                if vrf is None:
                    self.write_line(u"Error - VRF({}) does not exist or Route-Distinguisher not specified or Address Family not configured".format(
                        args[1]))
                else:
                    self.port.vrf = vrf
                    self.write_line(u"Warning: All IPv4 and IPv6 addresses (including link-local) on this interface have been removed")
    ...
    def do_port_name(self, *args):
        self.port.description = u" ".join(args)
