...
class ConfigCommandProcessor(BaseCommandProcessor):
    def get_prompt(self):
        return u"SSH@%s(config)#" % self.switch_configuration.name

    def do_vlan(self, raw_number, *args):
        number = int(raw_number)
        if number < 0:
            self.write_line(u"Invalid input -> -1")
            self.write_line(u"Type ? for a list")
        elif number == 0:
            self.write_line(u"Error: vlan ID value 0 not allowed.")
        elif number > 4090:
            self.write_line(u"Error: vlan id %s is outside of allowed max of 4090" % raw_number)
        else:
    ...
    def do_interface(self, *args):
        ...
        if port:
            ...
            if u"ve".startswith(args[0]):
                self.write_line(u"Error - invalid virtual ethernet interface number.")
            else:
                [self.write_line(l) for l in explain_missing_port(u" ".join(args))]
    ...
    def do_no_interface(self, *args):
        ...
        if port:
            if isinstance(port, VlanPort):
                ...
                self.switch_configuration.add_port(self.switch_configuration.new(u"VlanPort", port.vlan_id, port.name))
