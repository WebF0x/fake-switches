...
class DellConfigureVlanCommandProcessor(BaseCommandProcessor):
    def get_prompt(self):
        return u"\n" + self.switch_configuration.name + u"(config-vlan)#"

    def do_vlan(self, *args):
        vlan_id = int(args[0])
        self.print_vlan_significant_delays_warning()
        self.write_line(u"")
        self.switch_configuration.add_vlan(self.switch_configuration.new(u"Vlan", vlan_id))
    ...
    def do_no_vlan(self, *args):
        ...
        if vlan is not None:
            self.write_line(u"If any of the VLANs being deleted are for access ports, the ports will be")
            self.write_line(u"unusable until it is assigned a VLAN that exists.")
            self.switch_configuration.remove_vlan(vlan)
        else:
            self.write_line(u"")
            self.write_line(u"These VLANs do not exist:  {}.".format(vlan_id))
